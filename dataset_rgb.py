...
class DataLoaderVal(Dataset):
    def __init__(self, rgb_dir):
        ...
        clean_files = sorted(os.listdir(os.path.join(rgb_dir, '0_real')))
        noisy_files = sorted(os.listdir(os.path.join(rgb_dir, '1_fake')))
        self.clean_filenames = [os.path.join(rgb_dir, '0_real', x) for x in clean_files]
        self.noisy_filenames = [os.path.join(rgb_dir, '1_fake', x) for x in noisy_files]
